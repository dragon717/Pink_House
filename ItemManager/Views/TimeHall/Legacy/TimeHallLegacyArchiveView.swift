...
// MARK: - 旧馆「馆藏档案」（Phase 4 归档）
//
// 旧四模式（编年史/风格喷绘/故事/搭配）+ 品牌列表 + 商品详情的完整只读实现，
// 自 TimeHallView 原文件整体迁入（逐字保留，仅壳层职责剥离）：
//   · 壳层（TimeHallView.swift）负责首屏店家上新挂载、「馆藏档案」入口显隐、
//     Tab 复位与收藏迁移；
//   · 本视图 onClose 回调替代原 showsLegacyArchive = false 的内部状态翻转。
// 过渡期入口保留一个使用周期；入口移除时本文件随 Midsummer/Rebuild 一起下线。
struct TimeHallLegacyArchiveView: View {
  /// 返回店家上新首屏（由壳层控制显隐）
  var onClose: () -> Void = {}
