...
class PetViewModel: ObservableObject {
    ...
    func reloadStatus() {
        ...
        self.status.dailyFishCoinEarned = newStatus.dailyFishCoinEarned
        self.status.dailyBoneCoinEarned = newStatus.dailyBoneCoinEarned
        self.status.isAutoWorkEnabled = newStatus.isAutoWorkEnabled
        self.status.autoWorkStrategy = newStatus.autoWorkStrategy
        self.status.autoWorkRewardMode = newStatus.autoWorkRewardMode
        // 也可以选择完全重载，视需求而定
    }
    ...
    func startJob(_ job: PetJob, isAutomatic: Bool = false, rewardCurrency: PetCurrency? = nil) {
        guard job != .none else {
            stopJob()
            return
        }
        ...
        let result = PetWorkStateMachine.startJob(
            ...
            allowReplacingExistingJob: true
        )
        ...
        if job != .none {
            showFloatingText(result.message, style: .custom(.blue))
        }
    }

    func stopJob(isInterrupted: Bool = false) {
        guard let stopResult = PetWorkStateMachine.stopJob(in: &status, isInterrupted: isInterrupted) else { return }
        jobIncomeAccumulator = 0.0
        saveStatus()
        presentWorkStopResult(stopResult)
    }

    private func presentWorkStopResult(_ stopResult: PetWorkStopResult) {
        if stopResult.wasInterrupted {
            ...
            // 显示文案
            showFloatingText(result.message, style: result.success ? floatingStyle(for: stopResult.rewardCurrency) : .warning)
        }
    }

    func updateAutoWorkEnabled(_ isEnabled: Bool) {
        let stopResult = PetWorkStateMachine.updateAutoWorkEnabled(isEnabled, in: &status)
        if stopResult != nil {
            jobIncomeAccumulator = 0.0
        }
        saveStatus()
    ...
    func updateAutoWorkRewardMode(_ rewardMode: PetAutoWorkRewardMode) {
        PetWorkStateMachine.updateAutoWorkRewardMode(rewardMode, in: &status)
        saveStatus()
    }

    var autoWorkStatusSummary: String {
        PetWorkStateMachine.autoWorkStatusSummary(for: status)
    }

    private func normalizedJobRewardCurrency(_ currency: PetCurrency) -> PetCurrency {
        PetWorkStateMachine.normalizedRewardCurrency(currency)
    }

    private func floatingStyle(for currency: PetCurrency) -> FloatingTextStyle {
        switch currency {
        case .meowCoin:
            return .meowCoin
        case .fishCoin:
    ...
    private func resolvedAutoWorkRewardCurrency() -> PetCurrency {
        PetWorkStateMachine.resolvedAutoWorkRewardCurrency(for: status)
    }

    private func remainingJobIncomeQuota(for currency: PetCurrency) -> Int {
        PetWorkStateMachine.remainingJobIncomeQuota(in: status, for: currency)
    }

    @discardableResult
    private func addJobIncome(_ amount: Int, currency: PetCurrency, isOfflineSimulation: Bool) -> Int {
        let actualEarned = PetWorkStateMachine.addJobIncome(amount, currency: currency, to: &status)
    ...
    private func shouldStartAutoWork(now: Date) -> Bool {
        PetWorkStateMachine.shouldStartAutoWork(
            status: status,
            currentStateIsIdle: currentState == .idle,
            currentMinute: Calendar.current.component(.minute, from: now),
            forceSleepMinuteStart: forceSleepMinuteStart
        )
    }

    private func startAutoWorkIfNeeded(now: Date, isOfflineSimulation: Bool) {
        guard shouldStartAutoWork(now: now) else { return }
        guard let result = PetWorkStateMachine.startAutoWorkIfNeeded(
            in: &status,
            currentStateIsIdle: currentState == .idle,
            currentMinute: Calendar.current.component(.minute, from: now),
            forceSleepMinuteStart: forceSleepMinuteStart,
    ...
    private func shouldStopAutoWorkForProtection() -> Bool {
        PetWorkStateMachine.shouldStopAutoWorkForProtection(status: status)
    }
    ...
    func processTimePassage(timeInterval: Double, isOfflineSimulation: Bool = false) {
        ...
        // 打工中断检查：精力或者饱食归0会中断，自动打工达到保护线会正常下班
        let shouldInterruptWork = status.currentJob != .none && (status.energy <= 0 || status.hunger <= 0)
        let shouldStopProtectedAutoWork = status.currentJob != .none && shouldStopAutoWorkForProtection()
        let shouldStopWork = shouldInterruptWork || shouldStopProtectedAutoWork

        // 状态判定优先级：打工中断 > 精力耗尽 > 强制休息 > 工作 > 低精力自动休息

        if shouldStopWork {
            // 停止工作：手动工作只在耗尽时中断，自动工作会在保护线处正常下班
            if !isOfflineSimulation {
                stopJob(isInterrupted: shouldInterruptWork)
            } else {
                _ = PetWorkStateMachine.stopJob(in: &status, isInterrupted: shouldInterruptWork)
                jobIncomeAccumulator = 0.0
            }
            ...
            // 闲时自动打工：只在状态足够好、非强制休息时进入现有打工链路。
            startAutoWorkIfNeeded(now: now, isOfflineSimulation: isOfflineSimulation)
            if status.currentJob != .none {
                isWorking = true
        ...
        if isSleeping {
            ...
            if jobIncomeAccumulator >= 1.0 {
                ...
                _ = addJobIncome(
                    coinToAdd,
                    currency: status.currentJobRewardCurrency,
