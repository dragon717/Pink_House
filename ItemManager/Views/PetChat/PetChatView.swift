...
struct PetChatView: View {
    ...
    private var latestVisibleAssistantMessageID: UUID? {
        visibleMessages.last(where: { !$0.isUser })?.id
    }
    ...
    private func reusableWelcomeText() -> String {
        ...
        return "%1$@%2$@ 我是你的衣橱管家%3$@，有什么可以帮你的吗？".appLocalized(
            greeting,
    ...
    private var lockedWelcomeBubble: some View {
        PetChatBubble(
            ...
            assistantExpressionVideoName: petDialogueResolvedVideoName,
    ...
    private func handleHistoryUnlockDragEnded(_ value: DragGesture.Value) {
        ...
        guard isChatScrollPinnedToTop else {
            resetHistoryUnlockHintIfNeeded(animated: true)
            return
        }
    ...
    private func messageBubble(for message: PetChatMessage) -> some View {
        let expressionVideoName: String? = (message.id == latestVisibleAssistantMessageID && !message.isUser)
