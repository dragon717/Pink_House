...
### B1. 区分“内存已有”还是“本次云同步得到”

当前运行时公告列表来自 `NoticeService` 内存集合，不再从旧 SwiftData `Notice` 缓存启动读取。
...
- 看到内存列表有公告，不能单独作为云同步成功证据
- 只有日志里出现“成功拉取 X 条云端公告”才算本次云同步成功

### B2. 当前 iOS 17 止血约束

旧 TestFlight 安装可能留下 pre-CMS `Notice` SwiftData 表结构。iOS 17 真机在 `context.fetch(FetchDescriptor<Notice>)` 时可能直接由 CoreData abort，`do/catch` 接不住。

当前约束：

1. 公告运行时以 CloudKit + 内存集合为准
2. 启动链路不要读取旧 SwiftData `Notice` 缓存
3. `NoticeCloudKitService.syncNotices(with:)` 只保留云端拉取，不再合并本地 SwiftData 缓存
...
若代码层顺序变成“先结束 syncing，再刷新内存列表”，就容易出现：
