...
3. `ItemManager/Models/FavoriteMenuSettings.swift`
4. `ItemManager/Views/FavoriteMenuSettingsView.swift`
5. `ItemManager/Views/SmallWorldMenuOverlay.swift`
...
- 底部固定保留：衣橱、House、我。
- 底部最右侧为自定义快捷入口，默认萌宠对话。
- House 长按常用菜单先保留，设置入口升级为“常用入口”，兼容旧新手引导。
...
- 新增底部快捷入口设置管理器，持久化 key 使用新 key，不复用常用菜单历史。
- `MainTabView` 最右侧 tab 改为读取用户选择。
- `FavoriteMenuSettingsView` 顶部增加“底部快捷入口”设置区。
- `MeView` 入口文案改为“常用入口”。
...
cd "$PROJ"
git diff --check
rg -n "bottomDockSelectedFeature|AppFeatureRegistry|常用入口" ItemManager temp/产品框架体验
```
