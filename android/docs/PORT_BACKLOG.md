...
无 — BATCH-M4-03A 已验证；下一批建议进入 M4-03B local_notification Room schema 单独批。
...
- [ ] **BATCH-M4-03B** Notice `local_notification` Room schema（F-36 / F-38）
  - SCOPE: schema 单独批；新增本地通知/公告表，数据库 v4 migration，更新 Room schema JSON。

- [ ] **BATCH-M4-03C** NoticeCenterRoute 本地表查询 + 已读/ACK 持久化
  - 依赖：M4-03B。把 M4-03A 的样例公告替换为本地表数据。
