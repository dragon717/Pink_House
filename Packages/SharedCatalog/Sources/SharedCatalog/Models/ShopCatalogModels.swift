...
public enum CatalogSaleEventType: String, Codable, CaseIterable, Identifiable {
...
public enum CatalogCurrency: String, Codable, CaseIterable, Identifiable, Sendable {
    case cny = "CNY"
    case jpy = "JPY"
    ...
    public var clothingCurrencyCode: String {
        switch self {
        case .cny: return "CNY"
        case .jpy: return "JPY"
...
public nonisolated struct CatalogMoney: Hashable, Sendable {
    public var amount: Decimal
    public var currency: CatalogCurrency

    // 跨模块构造入口：`public` 结构体的合成逐成员 init 是 internal，外部模块必须显式声明。
    public init(amount: Decimal, currency: CatalogCurrency) {
        self.amount = amount
        self.currency = currency
    }

    public func isSameCurrency(as other: CatalogMoney) -> Bool {
...
public enum CatalogAssetType: String, Codable, CaseIterable, Identifiable {
...
public enum CatalogPublicationStatus: String, Codable, CaseIterable, Identifiable {
    case draft     // 草稿
    case submitted // 提交
    case reviewed  // 审核
    case published // 发布
...
public struct CatalogShop: Codable, Identifiable, Hashable, Sendable {
    ...
    public func matches(nameOrAlias query: String) -> Bool {
...
public nonisolated enum CatalogSeriesSalePhase: String, Codable, CaseIterable, Identifiable, Sendable {
...
public nonisolated enum CatalogBalanceDueKind: String, Codable, CaseIterable, Identifiable, Sendable {
...
public struct CatalogSeries: Codable, Identifiable, Hashable, Sendable {
    public var id: String
    ...
    public var priceChart: CatalogPriceChart? = nil
    ...
    public var salePhase: CatalogSeriesSalePhase? = nil
    ...
    public var balanceDueEndAt: Date? = nil
    ...
    public init(
        ...
        archivedAt: Date? = nil,
        priceChart: CatalogPriceChart? = nil,
        salePhase: CatalogSeriesSalePhase? = nil,
        reservationStartAt: Date? = nil,
        reservationEndAt: Date? = nil,
        balanceDueKind: CatalogBalanceDueKind? = nil,
        balanceDueText: String? = nil,
        balanceDueAt: Date? = nil,
        balanceDueEndText: String? = nil,
        balanceDueEndAt: Date? = nil
    ) {
        self.id = id
        self.shopID = shopID
        self.name = name
        ...
        self.salePhase = salePhase
        self.reservationStartAt = reservationStartAt
        self.reservationEndAt = reservationEndAt
        self.balanceDueKind = balanceDueKind
        self.balanceDueText = balanceDueText
        self.balanceDueAt = balanceDueAt
        self.balanceDueEndText = balanceDueEndText
        self.balanceDueEndAt = balanceDueEndAt
    }
}
...
public struct CatalogPriceCorrection: Codable, Hashable, Sendable {
    public var reservationPrice: Decimal? = nil
    public var stockPrice: Decimal? = nil
    public var deposit: Decimal? = nil
    public var balance: Decimal? = nil
    /// 最后一次修正时间（审计用；不是业务批次时间）
    public var correctedAt: Date? = nil
    /// 修正快照的币种（R02）。nil = 沿用销售记录的币种；
    /// 修正价格时必须与商品既有币种一致——修价不是换币种，跨币种修正会被拒绝。
    public var currency: CatalogCurrency? = nil

    // 跨模块构造入口：`public` 结构体的合成逐成员 init 是 internal，外部模块必须显式声明。
    public init(
        reservationPrice: Decimal? = nil,
...
public struct CatalogProduct: Codable, Identifiable, Hashable, Sendable {
    public var id: String
    public var shopID: String
    ...
    /// Optional 字段由合成 Decodable 以 decodeIfPresent 处理，旧 JSON 缺键自动置 nil。
    public var priceCorrection: CatalogPriceCorrection? = nil
    /// 款式名（2026-09-22「同款不同色」归类）：同款式不同颜色的商品共享同一款式名，
    /// 列表按款式合并展示、卡片内颜色子项切换。nil = 未显式指定（按商品名剥离颜色词派生）。
    /// Optional 字段由合成 Decodable 以 decodeIfPresent 处理，旧 JSON 缺键自动置 nil。
    public var designName: String? = nil

    /// 兼容旧格式：images 缺失时兜底为空数组（理由同 CatalogShop.init(from:)）
    public init(id: String, shopID: String, seriesID: String, name: String,
...
public struct CatalogProductVariant: Codable, Identifiable, Hashable, Sendable {
    public var id: String
...
public struct CatalogSizeRow: Codable, Hashable, Sendable {
...
public struct CatalogSizeChart: Codable, Identifiable, Hashable, Sendable {
    public var id: String
    public var productID: String
    public var unit: String? = nil
    public var columns: [String] = []
    public var rows: [CatalogSizeRow] = []
    /// 原始尺码表图片（CatalogAsset id，type = sizeChartImage）
    public var sourceImage: String? = nil
...
public struct CatalogStyleProfile: Codable, Identifiable, Hashable, Sendable {
    ...
    /// 面料（如「雪花提花布 + 蕾丝拼接」）
    public var fabric: String? = nil
    /// 款式描述（款式级共用文案）
    public var styleDescription: String? = nil
...
///     Optional 字段由合成 Decodable 以 decodeIfPresent 处理，旧 JSON 缺键自动置 nil。
public struct CatalogPriceChart: Codable, Hashable, Sendable {
    public var id: String
    public var seriesID: String
    public var unit: String? = nil
    public var columns: [String] = []
    public var rows: [CatalogSizeRow] = []
    public var sourceImage: String? = nil
    public var sourceImages: [String]? = nil
    ...
    public var hasStructuredContent: Bool { !columns.isEmpty && !rows.isEmpty }
}
...
public struct CatalogSaleEvent: Codable, Identifiable, Hashable, Sendable {
    public var id: String
    public var productID: String
    public var type: CatalogSaleEventType
    ...
    public var batchLabel: String? = nil
    ...
    public var priceTierMin: Decimal? = nil
    public var priceTierMax: Decimal? = nil
    ...
    public init(
        ...
        recordedAt: Date? = nil,
        currency: CatalogCurrency? = nil,
        priceTierMin: Decimal? = nil,
        priceTierMax: Decimal? = nil
    ) {
        self.id = id
        self.productID = productID
        self.type = type
        ...
        self.currency = currency
        self.priceTierMin = priceTierMin
        self.priceTierMax = priceTierMax
    }

    /// 生效币种：nil（旧数据）按「待确认」处理，**不做全局默认 CNY**。
    public var effectiveCurrency: CatalogCurrency { currency ?? .unknown }

    /// 是否为分档价格（有多个真实档位）
    public var hasPriceTier: Bool {
        guard let min = priceTierMin, let max = priceTierMax else { return false }
        return max > min
    }

    /// 校验定金 + 尾款与总价的一致性（允许缺省字段，不做强约束）
    public var isDepositBalanceConsistent: Bool {
    ...
    public var appendFingerprint: String {
    ...
    public var batchDisplay: String {
...
public struct CatalogAsset: Codable, Identifiable, Hashable, Sendable {
    public var id: String
    public var type: CatalogAssetType
...
public struct ShopCatalog: Codable, Hashable, Sendable {
    public var version: Int = 1
    public var shops: [CatalogShop] = []
    public var series: [CatalogSeries] = []
    public var products: [CatalogProduct] = []
    public var variants: [CatalogProductVariant] = []
    public var sizeCharts: [CatalogSizeChart] = []
    public var saleEvents: [CatalogSaleEvent] = []
    public var assets: [CatalogAsset] = []
    /// 款式（SPU）公共档案（2026-09-23）：面料 / 款式描述，一个款式一份。
    /// Optional 缺键容错：合成解码对带默认值字段走 decodeIfPresent（见下方 init(from:)）
    public var styleProfiles: [CatalogStyleProfile] = []
    /// 删除墓碑（2026-09-24 强制删除）：种子实体物理删不掉（Bundle 只读），
    /// 运营「强制删除」把 id 记进这里，合并层据此排除（含种子与覆盖层副本）。
    /// Optional 缺键容错：旧覆盖层 JSON 没有这些键也能解码。
    public var removedShopIDs: [String] = []
    public var removedSeriesIDs: [String] = []
    public var removedProductIDs: [String] = []

    /// 兼容旧格式：任何集合字段缺失时兜底为空数组（理由同 CatalogShop.init(from:)）
    public init(version: Int = 1, shops: [CatalogShop] = [], series: [CatalogSeries] = [],
...
public struct CatalogPriceArchive: Hashable, Sendable {
    /// 最近一次预约记录（**未修正**，来自 append-only 历史）
    public let reservation: CatalogSaleEvent?
    /// 最近一次现货记录（**未修正**，来自 append-only 历史）
    public let stock: CatalogSaleEvent?
    /// 价格修正结果（nil = 从未修正 → 下面所有「当前值」回退到历史推导）
    public let correction: CatalogPriceCorrection?

    public init(events: [CatalogSaleEvent], correction: CatalogPriceCorrection? = nil) {
    ...
    /// 是否存在价格修正
    public var isCorrected: Bool { correction != nil }
    ...
    /// 当前现货价
    public var currentStockPrice: Decimal? {
        if let correction { return correction.stockPrice }
        return stock?.price
    }

    /// 当前预约价
    public var currentReservationPrice: Decimal? {
        if let correction { return correction.reservationPrice }
        return reservation?.price
    }

    /// 历史预约价（计划 §6 用户页「历史预约价」）：保留旧命名，语义 = 当前生效预约价
    public var historicalReservationPrice: Decimal? { currentReservationPrice }

    /// 当前定金
    public var currentDeposit: Decimal? {
    ...
    public var reservationCurrency: CatalogCurrency? { reservation?.effectiveCurrency }

    /// 现货记录的生效币种（无现货记录时为 nil）
    public var stockCurrency: CatalogCurrency? { stock?.effectiveCurrency }

    /// 修正快照声明的币种
    public var correctionCurrency: CatalogCurrency? { correction?.currency }

    /// 当前生效币种：修正声明 > 最近一次销售记录；都没有则 nil（未知）。
    /// 预约与现货币种不一致时取**预约**（定金尾款口径以预约为准）。
    public var currentCurrency: CatalogCurrency? {
    ...
    public var isCrossCurrency: Bool {
    ...
    public var isMixedKnownUnknownCurrency: Bool {
        guard let r = reservationCurrency, let s = stockCurrency else { return false }
    ...
    public var stockOverReservationDelta: Decimal? {
    ...
    public var stockOverReservationDeltaPercent: Double? {
    ...
    public var deltaUnavailableReason: String? {
